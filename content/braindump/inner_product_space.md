...
An inner product space is a [Vector Space]({{< relref "linear_algebra.md#vector-space" >}}) \\(V\\) over a field \\(F\\) which is equipped with an inner product \\(\langle \cdot, \cdot \rangle: V \times V \mapsto F\\) and satisfies the following for \\(u, v \in V\\) and \\(c \in F\\):
